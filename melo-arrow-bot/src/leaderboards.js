...
function buildLeaderboardEmbeds(rows, periodType, periodStart, timezone) {
  ...
  const makeLines = (compact) => visibleRows.map((row, index) => {
    ...
    return `**${index + 1}. ${name} — ${arrows} arrows**`;
  });
...
class LeaderboardManager {
  ...
  schedule() {
    // Throttle rather than debounce: a constant stream of deliveries cannot
    // keep postponing the update. The board refreshes at most once per delay.
