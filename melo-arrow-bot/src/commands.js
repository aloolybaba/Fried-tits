...
const { buildLeaderboardPayload } = require('./leaderboards');
...
function createCommandHandler({ client, db, donutApi, config, leaderboards }) {
  async function execute({ command, args, guildId, userId, member, channel, commandMessage, respond, defer, edit }) {
    ...
    if (name === 'leaderboard') {
      ...
      return edit(buildLeaderboardPayload(rows, periodType, period.start, config.timezone));
    }
    ...
    if (name === 'add') {
      ...
      await db.addDelivery({ guildId, ign, amount, sourceType: 'manual', recordedBy: userId });
    ...
    if (name === 'refreshleaders') {
      ...
      return edit({ content: '✅ Latest completed hourly and daily leaderboard snapshots checked.' });
    }
