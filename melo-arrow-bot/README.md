...
| `LEADERBOARD_UPDATE_DELAY_MS` | `1000` by default; refreshes about one second after a new delivery while throttling Discord edits |
...
If a leaderboard channel already contains an active message created by this bot, its ID is remembered in PostgreSQL and that same single message is edited forever—even when the Riyadh hour/day changes. All 100 ranked users are shown as normal bold lines inside one embed, with unpadded ranks (`1`, `2`, `3`) instead of a code-block table. A player is displayed only after their current period total is greater than `MIN_ARROWS_TO_KEEP`.
