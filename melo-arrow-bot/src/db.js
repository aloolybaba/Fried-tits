...
class Database {
  ...
  async getLeaderboard(guildId, periodType, periodOverride = null, limit = 100) {
    let bounds;
    if (periodOverride) {
    ...
    const result = await this.pool.query(
      `SELECT normalized_ign, MAX(display_ign) AS display_ign, SUM(amount)::text AS total, COUNT(*)::int AS deliveries
       ...
       LIMIT $4`,
      [guildId, bounds.start, bounds.end, limit],
    );
    return result.rows;
  }
  ...
  async getLeaderboardMessage(guildId, periodType, periodStart) {
    const result = await this.pool.query(
      `SELECT message_id, channel_id FROM leaderboard_messages
  ...
  async getLatestLeaderboardPeriod(guildId, periodType) {
    const result = await this.pool.query(
      `SELECT period_start FROM leaderboard_messages
       WHERE guild_id = $1 AND period_type = $2
       ORDER BY period_start DESC LIMIT 1`,
      [guildId, periodType],
    );
    return result.rows[0]?.period_start || null;
  }

  async getLatestLeaderboardMessage(guildId, periodType) {
    const result = await this.pool.query(
      `SELECT message_id, channel_id, period_start FROM leaderboard_messages
       WHERE guild_id = $1 AND period_type = $2
       ORDER BY period_start DESC LIMIT 1`,
      [guildId, periodType],
    );
  ...
  async getLeaderboardMessageIds(guildId, periodType) {
    const result = await this.pool.query(
      `SELECT DISTINCT message_id FROM leaderboard_messages
       WHERE guild_id = $1 AND period_type = $2`,
      [guildId, periodType],
    );
  ...
  async pruneLeaderboardMessages(guildId, periodType, keepMessageId) {
    await this.pool.query(
      `DELETE FROM leaderboard_messages
       WHERE guild_id = $1 AND period_type = $2 AND message_id <> $3`,
      [guildId, periodType, keepMessageId],
    );
  }

  async saveLeaderboardMessage(guildId, periodType, periodStart, messageId, channelId) {
    await this.pool.query(
      `INSERT INTO leaderboard_messages (guild_id, period_type, period_start, message_id, channel_id)
       VALUES ($1, $2, $3, $4, $5)
       ON CONFLICT (guild_id, period_type, period_start) DO UPDATE
       SET message_id = EXCLUDED.message_id, channel_id = EXCLUDED.channel_id`,
      [guildId, periodType, periodStart, messageId, channelId],
    );
  }
