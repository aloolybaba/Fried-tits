...
The bot records deliveries immediately but does not continuously edit the statistics channels. When a Riyadh hour finishes, it posts one new finalized hourly snapshot; when a Riyadh day finishes, it posts one new finalized daily snapshot. Older posts are kept unchanged. On startup, it also checks that the most recently completed hour and day have snapshots without creating duplicates. The top 100 is divided into 10-player pages with First, Previous, Next, and Last buttons. Each button remembers the exact historical hour/day, so pagination on an older post continues to show that older period. Players are shown as normal bold lines with unpadded ranks (`1`, `2`, `3`) instead of a code-block table. A player is displayed only after their period total is greater than `MIN_ARROWS_TO_KEEP`.
